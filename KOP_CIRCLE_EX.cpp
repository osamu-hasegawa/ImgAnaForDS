...
#endif
//2番目領域
	{ 0, 8  , IDC_STATIC61, 3, "測定結果②"        ,  10, 599, 300, 22, WS_BORDER|SS_CENTERIMAGE},
...
//3番目領域
	{ 0, 8  , IDC_STATIC67, 3, "測定結果③"        ,  10, 675, 300, 22, WS_BORDER|SS_CENTERIMAGE},
...
//4番目領域
	{ 0, 8  , IDC_STATIC73, 3, "測定結果④"        ,  10, 751, 300, 22, WS_BORDER|SS_CENTERIMAGE},
...
//5番目領域
	{ 0, 8  , IDC_STATIC79, 3, "測定結果⑤"        ,  10, 827, 300, 22, WS_BORDER|SS_CENTERIMAGE},
...
//6番目領域
	{ 0, 8  , IDC_STATIC85, 3, "測定結果⑥"        ,  10, 903, 300, 22, WS_BORDER|SS_CENTERIMAGE},
...
//7番目領域
	{ 0, 8  , IDC_STATIC91, 3, "測定結果⑦"        ,  10, 979, 300, 22, WS_BORDER|SS_CENTERIMAGE},
...
//8番目領域
	{ 0, 8  , IDC_STATIC97, 3, "測定結果⑧"        ,  10, 1055, 300, 22, WS_BORDER|SS_CENTERIMAGE},
...
//9番目領域
	{ 0, 8  , IDC_STATIC103, 3, "測定結果⑨"        ,  10, 1131, 300, 22, WS_BORDER|SS_CENTERIMAGE},
...
#if 0
		CString buf;
		buf.Format("エリア(%d,%d)-(%d,%d)", m_d.P_AREA[0].left, m_d.P_AREA[0].top, m_d.P_AREA[0].right, m_d.P_AREA[0].bottom);
...
#if 0
	buf1.Format("重心 (%.1lf,%.1lf)", m_d.R_COG_X[i][m_d.P_CUR_AREA], m_d.R_COG_Y[i][m_d.P_CUR_AREA]);
	buf2.Format("面積 %.0lf pix", m_d.R_SQAR[i][m_d.P_CUR_AREA]);
	buf3.Format("中心からのズレ(%4.1lf,%4.1lf)", m_d.R_DIF_X[i][m_d.P_CUR_AREA], m_d.R_DIF_Y[i][m_d.P_CUR_AREA]);
	buf4.Format("直径  %.2lf pix", m_d.R_PHI[i][m_d.P_CUR_AREA]);
#else
		for(int j = 0; j < 9; j++)
		{
			if(m_d.isEnable[j] == 0)
